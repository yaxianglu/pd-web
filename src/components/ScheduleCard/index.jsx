...
import { Button, Calendar, DatePicker, Modal, Select, Space, TimePicker, Tooltip, Input, Form, message, Table, Tag, Progress } from "antd";
...
export default function ScheduleCard({
  ...
  const onSelectStaffFile = useCallback(async (e) => {
    ...
    try {
      setIsUploading(true);
      setUploadProgress(0);
      messageApi.loading({ content: '正在上傳文件...', key: uploadKey });
      ...
      const onProgress = ({ progress }) => {
        ...
        messageApi.loading({
          content: `上傳進度: ${Math.round(progress)}%`,
          key: uploadKey
        });
      };
      ...
      if (isLargeFile) {
        // 大文件使用分块上传
        result = await api.uploadLargeFile(
        ...
        // 小文件使用二进制直接上传
        result = await api.uploadBinaryFile(
          `/api/smile-test/uuid/${smileTestUuid}/upload-file`,
          file,
          { onProgress, metadata }
        );
      ...
      messageApi.success({ content: '文件已上傳', key: uploadKey, duration: 2 });
      Modal.success({ 
        title: '提示', 
        content: `文件 "${file.name}" 已成功上傳`, 
        centered: true 
      });
      ...
      {currentPatient && (
        <div className="schedule-card-footer-tools">
          ...
          <div className="schedule-card-footer-tools-buttons">
            ...
            >
              {isUploading ? `上傳中 ${uploadProgress}%` : '上傳'}
            </button>
            <button
              type="button"
